...
public:
    ...
    System(const Matrix<T,size> & m, const Vector<T,size> & v): mat(m), vec(v) {} // konstruktor przyjmujacy konkretna macierz i konkretny wektor
...
template <typename T, int size>
std::ostream & operator << (std::ostream & o, const System<T,size> & u); // przeciazony operator << dla klasy System
template <typename T, int size>
std::istream & operator >> (std::istream & i, System<T,size> & u); // przeciazony operator >> dla klasy System
